### 1.事务中用（本事务）快照体现了很强的隔离性（不受其他事务的影响的程度），自然就解决了不可重复读问题，使用快照读的时候不会出现幻读，而使用当前读出现幻读
...
如果事务中的语句不先再内存缓存，而是直接写入，回导致一部分操作持久下来，剩下的可能由于故障而没有写入磁盘，这就违反了原子性--要么都做，要么都不做
### 14.一个很好的例子
<img width="366" height="334" alt="eaae9a1d3b2c7ce619487d05f5e71ea0" src="https://github.com/user-attachments/assets/40f99ac3-2854-4ec1-99c4-b085cca10557" />
首先gap lock是不冲突的，所以事务A,B的update即使都占有（E,W)的gap lock他们并不冲突 
<img width="1080" height="236" alt="image" src="https://github.com/user-attachments/assets/c18396c4-59f3-4b16-9374-02679b3aa3e4" />
