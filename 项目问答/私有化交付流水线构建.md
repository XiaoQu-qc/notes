...
### 5.实现接口访问鉴权逻辑
在完成项目的私有化镜像构建之后，又实现接口访问鉴权逻辑

### 6.jina-reranker-v2私有化部署
这个私有化部署分为三个不同的架构，需要分别分别进行配置，编写脚本，
#### 1.x86_nvidia 容器启动成功但是外部用10000端口请求访问时，发现vllm报config not eq错误,之后直接启动容器，在容器里面测试了8000端口（不通），8001端口（通），了解项目的端口转发的路径后发现，10000端口是外部请求模型服务的接口，8001是代理 / 网关服务在容器内部监听的端口（负责转发请求到模型服务）容器内的代理服务（Gin 框架）.并且结合docker ps的输出信息端口映射10000-->8000发现本应该映射到8001端口却没有映射到。

打包好的代码.tar（有模型包，vllm镜像）发到kas的宿主机，用ssh远程链接，宿主机里面启动容器验证功能。在容器里面直接修改start.sh文件，可以在容器内直接运行 start.sh，看是否能正常启动 vLLM，这是修改过的。，但容器重建后会丢失。
临时在容器中修改，进入容器，像平常一样修改文件，但是要把修改的文件复制出来到宿主机上，然后删除容器，
