...
原先再dockerfile中RUN pip3 config set global.index-url https://mirrors.tuna.tsinghua.edu.cn/pypi/web/simple/ \
    ...
    后面两个wps在构建镜像过程中无法连接，而其中的pdfcv这个库只在公司镜像中有，下载pdfcv并添加到远程仓库中package目录下，修改dockerfile run pip命令
```
RUN pip3 install --no-cache-dir -r requirements.txt

COPY package/ /tmp/package/
RUN pip3 install --no-cache-dir /tmp/package/*.whl && rm -rf /tmp/package
```
这样在构建镜像时，不在通过网络请求获取这些依赖包
